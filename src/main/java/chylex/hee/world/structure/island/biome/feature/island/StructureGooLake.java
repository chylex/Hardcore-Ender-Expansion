...
public class StructureGooLake extends AbstractIslandStructure{
	...
	protected boolean generate(Random rand){
		...
		for(int testX = (int)Math.floor(xx-rad); testX <= xx+rad; testX++){
			for(int testZ = (int)Math.floor(zz-rad); testZ <= zz+rad; testZ++){
				boolean found = false;
				
				for(int testY = yy+2; testY >= yy-2; testY--){
		...
		for(int a = 0; a < 3+rad+rand.nextInt(5); a++,yOff = 0){
			double cx = xx+Math.min(hrad,Math.abs(rand.nextGaussian()*0.7D*a))*(rand.nextInt(2)*2D-1D),
				   cz = zz+Math.min(hrad,Math.abs(rand.nextGaussian()*0.7D*a))*(rand.nextInt(2)*2D-1D),
				...
				for(int px = (int)Math.floor(cx-r); px <= cx+r; px++){
					for(int pz = (int)Math.floor(cz-r); pz <= cz+r; pz++){
						if (MathUtil.distance(px-cx,pz-cz) < r-0.3D+rand.nextDouble()*0.7D){
							for(int py = minY+yOff; py < yy+2; py++){
