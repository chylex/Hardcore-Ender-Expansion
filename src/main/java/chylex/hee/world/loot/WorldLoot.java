...
public class WorldLoot implements IVillageTradeHandler{
	...
	public static void registerWorldLoot(){
		...
		// KNOWLEDGE FRAGMENTS
		item = new WeightedRandomKnowledgeFragment(6);
		for(String s:new String[]{
			DUNGEON_CHEST, PYRAMID_DESERT_CHEST, PYRAMID_JUNGLE_CHEST, VILLAGE_BLACKSMITH
		})getInfo(s).addItem(item);
		
		item = new WeightedRandomKnowledgeFragment(7);
		getInfo(MINESHAFT_CORRIDOR).addItem(item);
		
		item = new WeightedRandomKnowledgeFragment(9);
		for(String s:new String[]{
	...
	public void manipulateTradesForVillager(EntityVillager villager, MerchantRecipeList recipeList, Random rand){
		if (villager.getProfession() == 1){
			if (rand.nextFloat() < 0.65F)recipeList.add(new MerchantRecipe(new ItemStack(Items.emerald,2+rand.nextInt(3)),ItemKnowledgeNote.setRandomFragment(new ItemStack(ItemList.knowledge_note),rand)));
