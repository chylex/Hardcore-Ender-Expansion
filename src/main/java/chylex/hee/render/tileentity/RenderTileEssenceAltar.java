...
public class RenderTileEssenceAltar extends TileEntitySpecialRenderer{
	...
	public void renderTileEntityAt(TileEntity tile, double x, double y, double z, float partialTickTime, int what){
		...
		Tessellator.getInstance().getWorldRenderer().setColorOpaque_F(1F,1F,1F);
		int l = 15728880;
		...
		viewRot = 180D+Math.toDegrees(Math.atan2(renderManager.viewerPosX-altar.getPos().getX()-0.5D,renderManager.viewerPosZ-altar.getPos().getZ()-0.5D));
	...
	private void renderAltar(){
		...
		for(int a = 0; a < sockets.length; a++){
			...
			Minecraft.getMinecraft().getBlockRendererDispatcher().renderBlock(Block.getBlockFromItem(sockets[a].getItem()).getDefaultState(),altar.getPos(),getWorld(),Tessellator.getInstance().getWorldRenderer());
			GL11.glPopMatrix();
			GL11.glPopMatrix();
		}
	}
	...
	private void renderItem(Item item, int damage, float red, float green, float blue){
		...
		Minecraft.getMinecraft().getRenderItem().renderItemModel(new ItemStack(item)); // TODO
	}
}
